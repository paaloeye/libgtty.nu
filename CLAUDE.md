...
subcommands: `gtty enter`, `gtty exit`, and `gtty siblings`.
...
(Nushell's in-memory SQLite) under table `GTTY_PANES` with a 60-second TTL.
...
nu --no-config-file -c "
    ...
    stor delete --table-name GTTY_PANES
    stor create --table-name GTTY_PANES --columns { raw: str, cached_at: datetime }
    { raw: \"ME:2\n1|/dev/ttys001|kak README.md\n3|/dev/ttys003|zzz\n4|/dev/ttys004|gitui\n\",
      cached_at: (date now) } | stor insert --table-name GTTY_PANES
